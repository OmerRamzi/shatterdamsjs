...
export default function ClientQuoteDetailsPage() {
  ...
  const fetchQuote = () => {
    fetch(`/api/portal/client/quotes/${quoteId}`)
      .then(res => {
  ...
  if (isNaN(quoteId)) return <div className="p-6 text-destructive">Invalid Quote ID</div>;
  if (isLoading) return <div className="p-6">Loading quote details...</div>;
  if (!data) return <div className="p-6 text-destructive">Quote not found.</div>;
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          <Link to="/client/quotes" className="hover:text-foreground flex items-center gap-1 transition-colors">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <ClientQuoteAction quoteId={quote.id} currentStatus={quote.status} onStatusUpdate={fetchQuote} />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-12 border-y border-border/50 py-6">
          ...
          <div className="text-right">
            <div className="mb-4">
              ...
              <p className="font-medium text-sm">{new Date(quote.createdAt).toLocaleDateString()}</p>
            </div>
            {quote.validUntil && (
              <div>
