import { getClientQuotes } from "@/app/actions/portal";
import { FileText, ArrowRight } from "lucide-react";
import Link from "next/link";

export default async function ClientQuotesPage() {
  const quotesList = await getClientQuotes();
                ...
                quotesList.map(({ quote }) => (
                  <tr key={quote.id} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link href={`/client/quotes/${quote.id}`} className="inline-flex items-center gap-1.5 text-xs font-medium text-primary hover:bg-primary/10 px-3 py-1.5 rounded-md transition-colors">
