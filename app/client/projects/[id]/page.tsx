import { getClientProjectDetails, getClientFiles } from "@/app/actions/portal";
...
export default async function ClientProjectDetailsPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const projectFiles = await getClientFiles(projectId);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
              <Link href="/client/projects" className="hover:text-foreground transition-colors">My Projects</Link>
              <span>/</span>
              <span>Details</span>
            </div>
                  ...
                  projectFiles.map((file) => (
                    <div key={file.id} className="p-4 border-b last:border-0 border-border hover:bg-secondary/10 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <FileRowAction fileId={file.id} currentStatus={file.status!} userRole="client" />
                      </div>
