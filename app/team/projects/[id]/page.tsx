import { getTeamProjectDetails, getTeamFiles } from "@/app/actions/portal";
...
export default async function TeamProjectDetailsPage({ params }: { params: { id: string } }) {
  const projectId = parseInt(params.id);
  if (isNaN(projectId)) notFound();

  try {
    const { project, client } = await getTeamProjectDetails(projectId);
    const projectFiles = await getTeamFiles(projectId);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
              <Link href="/team/projects" className="hover:text-foreground transition-colors">My Projects</Link>
              <span>/</span>
