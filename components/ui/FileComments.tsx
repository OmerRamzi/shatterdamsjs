...
import { getFileComments, addFileComment } from "@/app/actions/files";
...
export function FileComments({ fileId }: { fileId: number }) {
  ...
  useEffect(() => {
    getFileComments(fileId).then(setComments).catch(console.error);
  }, [fileId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewComment("");
      // Refresh local state to feel snappy without full reload
      const updated = await getFileComments(fileId);
      setComments(updated);
      router.refresh();
    } catch (error) {
      console.error(error);
