...
import { getPresignedUploadUrl, registerUploadedFile } from "@/app/actions/files";
...
export function FileUpload({ projectId }: { projectId: number }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      // 1. Get Presigned URL
      const { uploadUrl, key } = await getPresignedUploadUrl(projectId, file.name, file.type);
      setProgress(30);

      // 2. Upload directly to R2 using XMLHttpRequest to track progress, or fetch. 
      // Using fetch here for simplicity, though XHR is better for real progress bars.
